(ns canvas.vocab.code.effect
  "Code vocab — `Effect`: a named side effect an Operation performs, the transitive
   ...
   This file keeps the Effect element itself — the value structure, the `effectful` property,
   extraction, and the analysis helpers. (The op pairing `op-twin` lives in `module`, referenced
...
(def reaches-effect-rules
  "Transitive effect reachability over the reified code graph: an op REACHES effect E if it directly
   ...
   NB: the by-name `reaches-effect-rules` (this reader-side reach) and the generated by-identity
   law (`:corresponds/Operation.performs-covered`, reaching via the character-generated `calls+`
   then `:performs`) are DELIBERATELY different computations — the reader keeps by-name for
   judgment surfaces; nothing to keep in sync."
