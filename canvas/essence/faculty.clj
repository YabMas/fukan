(ns canvas.essence.faculty
  "Essence grammar — what a `Faculty` IS: a core concept/capability of fukan in the
   top-level flow view. The vocabulary the essence view (`canvas.essence.overview`) is
   written in. (Split out of the old `canvas.vocab.arch`: `Faculty` is fukan-specific
   essence and lives here; the generic `Module`/`Connected` primitives moved to
