...
(Module canvas-source
  ...
  (Operation ^:private require-canvas-namespace "Require a namespace (throws on load failure)."
    {:signature [:=> [:catn [:ns-sym NsSymbol]] Unit]
     :performs  [:require :throws]})
