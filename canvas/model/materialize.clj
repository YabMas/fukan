...
(def materialize-view
  (Stage (in [db StructureDb]) (in [lens Lens]) (out Instruction)))  ; lens focus, Blueprint default
;; the focus-consuming entry — a refined focus (core.lens/refine) renders straight in
(def materialize-over
  (Stage (in [db StructureDb]) (in [projection ProjectionName]) (in [focus [Eid]])
    (out Instruction)))
(def materialize-focus
  (Stage (in [db StructureDb]) (in [projection ProjectionName]) (in [clauses [Clause]])
    (out Instruction)                                                                   ; ad-hoc focus
    (calls materialize-over lens-engine/focus-nodes)))
(def materialize-module
  (Stage (in [db StructureDb]) (in [projection ProjectionName]) (in [module ModuleName])
    (out Instruction)
    (calls materialize-focus)))
(def materialize-projection
  (Stage (in [db StructureDb]) (in [proj Projection]) (out Instruction)  ; model-driven
