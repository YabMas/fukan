...
;; ── the correspondence: Module ↦ Ns, one sort of the design→Clojure morphism ──
;; `Module :eq Ns` — a bijection (`:eq` ⇒ total + surjective: every design Module has an Ns twin and
;; vice-versa), the ROOT sort paired by name-match. Rides Module from OUTSIDE.
(s/correspond Module :eq Ns (bridge :qualified-suffix))
