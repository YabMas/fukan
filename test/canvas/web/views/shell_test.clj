(ns canvas.web.views.shell-test
  (:require [clojure.test :refer [deftest is]]
            [canvas.web.views.shell :as port]
...
(deftest canvas-build-has-expected-entities
  (let [db (port/build-canvas)
        names (->> (d/q '[:find ?n :where [?e :entity/name ?n]] db)
                   (map first)
                   set)]
    (is (contains? names "web.views.shell") "module present")
    (is (contains? names "render_app_shell") "render_app_shell function present")))
