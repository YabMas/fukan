(ns canvas.libs.boundary.parser-test
  (:require [clojure.test :refer [deftest is]]
            [canvas.libs.boundary.parser :as port]
...
(deftest canvas-build-has-expected-entities
  (let [db (port/build-canvas)
        names (->> (d/q '[:find ?n :where [?e :entity/name ?n]] db)
                   (map first)
                   set)]
    (is (contains? names "libs.boundary.parser") "module name present")
    (is (contains? names "ParsedBoundary") "ParsedBoundary record present")
    ...
    (is (contains? names "parse_boundary") "parse_boundary function present")
