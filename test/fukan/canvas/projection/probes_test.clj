...
(deftest the-full-probe-surface-runs
  (testing "all four leaves run over the self-model and yield observation findings"
    (let [db  (pipeline/build-model nil)
          all (probes/run-all db)]
      (is (= #{"survey" "patterns" "consistency" "callers"}
