(ns fukan.common.reflect.grammar-test
...
(defn- struct-node [db tag-str]
  (ffirst (cq/q '[:find ?s :in $ ?t
                 :where [?s :structure/of :fukan.common.reflect.grammar/Structure] [?s :val/tag ?t]]
               db tag-str)))

(deftest structures-reify-as-nodes
  (let [db (reflected)
        n  (struct-node db ":fukan.common.reflect.grammar-test/Node")]
...
(deftest slots-reify-as-card-kinded-labeled-edges
  (let [db (reflected)
    ...
    (is (= (struct-node db ":fukan.common.reflect.grammar-test/Leaf")
    ...
    (is (= #{"Leaf" "Node"}
           (set (cq/q '[:find [?n ...]
                       :where [?v :structure/of :fukan.common.reflect.grammar/Vocabulary]
                              [?v :entity/name "fukan.common.reflect.grammar-test"]
                              [?r :rel/from ?v] [?r :rel/kind :child] [?r :rel/to ?c]
                              [?c :entity/name ?n]]
                     db))))))

(deftest the-reflection-self-reifies
  (testing "the strange loop: fukan.common.reflect.grammar's own Structure gets a Structure node"
    (let [db (reflected)]
      (is (some? (struct-node db ":fukan.common.reflect.grammar/Structure")))
      (is (some? (struct-node db ":fukan.common.reflect.grammar/Law"))))))
