  ...
  (testing "the clj-kondo extractor emits an Operation per defn/defn-, with privacy"
    (let [db  (extract "test/fixtures/target/sample.clj")
          ops (into {} (cq/q '[:find ?n ?p
                              :where [?e :structure/of :fukan.common.vocab.code.operation/Operation]
  ...
  (testing "an annotated defn's :malli/schema is DECOMPOSED into :in/:out Schema nodes (no blob)"
    (let [db  (extract "test/fixtures/target/sample.clj")
          out (ffirst (cq/q '[:find ?k
                             :where [?e :structure/of :fukan.common.vocab.code.operation/Operation] [?e :entity/name "alpha"]
  ...
  (testing "each namespace becomes a Module that owns its Operations (via :child relations)"
    (let [db    (extract "test/fixtures/target/sample.clj")
          owned (cq/q '[:find ?mn ?on
                       :where [?m :structure/of :fukan.common.vocab.code.module/Module] [?m :entity/name ?mn]
                              [?r :rel/kind :child] [?r :rel/from ?m] [?r :rel/to ?o]
                              [?o :structure/of :fukan.common.vocab.code.operation/Operation] [?o :entity/name ?on]]
                     db)]
...
(deftest extracted-modules-carry-provenance
  (testing "each extracted Module is stamped :val/extracted true"
    (let [db (extract "test/fixtures/target/sample.clj")]
      (is (true? (ffirst (cq/q '[:find ?x :where [?m :structure/of :fukan.common.vocab.code.module/Module]
...
(deftest every-modelled-stage-is-realized-in-src
  (testing "fukan-on-itself: build-model unifies the authored self-model (canvas/)
    ...
    (let [model      (pipeline/build-model "src")        ; design + extracted code, unified
      ...
      (is (seq (cq/q '[:find ?s :where [?s :structure/of :fukan.common.vocab.code.operation/Operation]] model)) "model has Operations")
      (is (seq (cq/q '[:find ?o :where [?o :structure/of :fukan.common.vocab.code.operation/Operation]] model)) "build-model extracted code into Operations")
      (is (empty? unrealized)
