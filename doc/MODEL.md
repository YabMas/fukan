...
**Status:** ⚠️ **Design-phase substrate spec — largely superseded.** This document specifies the *kernel* substrate as it was designed: nine typed primitives, thirteen relations, the §5 vocabulary mechanism, the Expression/Effect calculus. The implemented substrate has since been **unified to a much smaller floor** (see *Current substrate* below). Read the kernel-primitive sections (§3.1–3.2, §4, §5) as history. The forward-looking sections remain the live design intent and are being carried forward: the **Type vocabulary** (§3.3), **Expression/Effect** (§3.8 — the seed of the deferred "behavioural depth" work), the **constraint language** (§6), the **projection vocabulary** (§7), and the **substrate commitments** (§11).

**Reading order:** Read [VISION.md](./VISION.md) for the current vision, then [DESIGN.md](./DESIGN.md) for the canvas layering and build pipeline. [DECISIONS.md](./DECISIONS.md) preserves the design-phase decision trace; K\*/R\*/V\*/C\*/P\* identifiers cited below resolve there.

## Current substrate (the unification)

The implemented substrate is **two primitives — Node and Relation**. Architectural *kinds* (the kernel's Container / Operation / Rule / Event / … and the older canvas's Module / Affordance / Type) are *not* primitives: a node carries its kind as an **introspectable tag-application**, and a **tag-definition registry** holds each vocabulary term as data (`{:tag :family :payload :edges :doc}`). A node's typed payload is reified into queryable datoms — shapes as a `:shape/*` tree, behavioural payloads as native edn (no `pr-str` blobs). Vocabularies **self-register** their terms and build nodes through a generic, registry-driven interpreter (the *construct-kit*) over a small production-directive vocabulary — so a vocabulary is a plugin, and the core knows no kinds. Where the kernel said "nine primitives connected by thirteen relations," the substrate now says "Node + Relation, classified by tags." The thirteen relation *names* and their semantics largely survive (as edges between nodes); the nine primitive *kinds* dissolved into the tag vocabulary.

The body below is retained as the design-phase kernel spec and as the home of the still-live sections noted above. Read it as history except where the current docs cross-reference it.
