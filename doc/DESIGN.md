...
- **Reference grammar**. `.boundary` adopts Allium's import-and-qualification shape verbatim. `use "<path>" as <alias>` at the file head declares cross-module aliases. **Type-refs, Event refs, and other named primitive references use dot syntax**: `alias.RuleName`, `alias.EntityName`, `alias.EventName`. The `.boundary` **foreign-attach `fn` form** keeps slash syntax to disambiguate alias-to-Contract from Contract-to-Operation: `fn alias/ContractName.operation_name { ... }` (the slash separates "outside this file" from "sub-part of that imported thing"; the dot separates Contract from its Operation). Inside a `subsystem` block, each entry in `contains:` implicitly binds the module's filename-stem as an alias.
